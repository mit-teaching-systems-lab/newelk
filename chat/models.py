...
class ChatNode(MPTTModel):
    ...
    feedback_link = models.ForeignKey(
        Feedback, on_delete=models.PROTECT, blank=True, null=True
    )
...
class Scenario(MPTTModel):
    ...
    visible_to_players = models.BooleanField(choices=BOOL_CHOICES, default=True)
...
class TFQuestion(models.Model):
    ...
    answer = models.BooleanField(choices=BOOL_CHOICES, default=True)
...
class OnboardLevel(models.Model):
    ...
    chat_tree = models.ForeignKey(
        ChatNode, on_delete=models.PROTECT, null=True, blank=True
    )
