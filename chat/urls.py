...
urlpatterns = [
    ...
    path(
        "<int:level>/singleplayerchat/",
        views.single_player_chat,
        name="single_player_chat_1",
    ),
    path(
        "<int:level>/singleplayerchat/<int:final_node>/",
        views.chat_feedback,
        name="single_player_chat_1",
    ),
    ...
    url(
        r"^(?P<role>[-\s\w]+)/(?P<scenario>[-\s\w]+)/(?P<room_name>[^/]+)/$",
        views.room,
        name="room",
    ),
    url(
        r"^(?P<role>[-\s\w]+)/(?P<scenario>[-\s\w]+)/(?P<room_name>[^/]+)/quiz/$",
        views.quiz,
        name="quiz",
    ),
]
